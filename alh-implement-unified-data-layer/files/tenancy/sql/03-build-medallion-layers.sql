...
DEFINE object_base_uri = 'Replace_With_Object_Storage_Base_URI'

PROMPT Stage 3 of 6: Building the curated medallion data and simulated operational evidence.
PROMPT Review and replace object_base_uri before running this script.

PROMPT Verifying the five Data Loader staging tables before the curated build.
SELECT 'FINANCIAL_ASSETS_LOAD' AS source_table, COUNT(*) AS row_count FROM admin.financial_assets_load
UNION ALL SELECT 'PURCHASE_ORDERS_LOAD', COUNT(*) FROM admin.purchase_orders_load
UNION ALL SELECT 'PROJECT_MILESTONES_LOAD', COUNT(*) FROM admin.project_milestones_load
UNION ALL SELECT 'SUPPLIER_EXTRACT_LOAD', COUNT(*) FROM admin.supplier_extract_load
UNION ALL SELECT 'INSPECTION_FINDINGS_LOAD', COUNT(*) FROM admin.inspection_findings_load;
...
COMMIT;
PROMPT Stage 3 complete: Curated medallion data, document metadata, and operational evidence are ready.
