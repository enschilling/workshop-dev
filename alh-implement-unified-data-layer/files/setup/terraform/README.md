...
- Permission to create Autonomous AI Database and Object Storage resources.
- A tenancy-level resource-principal policy, created outside this Terraform stack, that allows Autonomous AI Database principals to access the workshop Object Storage compartment.
...
This Terraform configuration does not create dynamic groups or IAM policies. Before running the database bootstrap, create the tenancy-level resource-principal policy described in Lab 1, Task 4. The policy must be in place before the `SEER_WORKSHOP` user retrieves the ONNX model from Object Storage.
...
- IAM policy propagation can take several minutes. Create the tenancy-level policy before running the model-loading stage.
